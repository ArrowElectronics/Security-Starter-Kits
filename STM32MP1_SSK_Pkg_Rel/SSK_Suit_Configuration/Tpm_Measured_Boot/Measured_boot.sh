...
cleanup() {
#    rm -f pcr_kernel_original.bin pcr_kernel_measured.bin pcr_rootfs_original.bin pcr_rootfs_measured.bin
...
sleep 2
if [ $(xxd -p pcr_kernel_original.bin | tr -d '\n' ) != $(xxd -p pcr_kernel_measured.bin | tr -d '\n' ) ]; then

...
#if 0
#include <stdio.h>
#define printf(...)       //printf(stderr, __VA_ARGS__)
#else
//#define //printf(...)
#endif
...
recv_t recv = {
		{0}, //pkt
		0, 	//len
		HW_BUFF_BASE_ADDR, 	//hw_buff_cur_off
		SW_BUFF_BASE_ADDR, 	//hw_buff_cur_off
		0  	//hw_ptr_cur_loc
};

uint8_t sock_create_resp = 0;
uint8_t sock_connect_resp = 0;
int8_t tx_done = 0;
uint8_t recv_cb_arrived = 0;
uint32_t offset  = 0;
uint8_t buff_available ;
uint8_t counter ;

/* ipv4 FQDN resolver callback*/
int resolveFQDNCallback(const xbee_cmd_response_t FAR *response);
/* check network carrier connected or not */
int xbeeConnectCallback(const xbee_cmd_response_t FAR *response);
/* fetch IMEI number of cellular modem */
int xbeeIMEICallback(const xbee_cmd_response_t FAR *response);

/* Send XBee FQDN command */
...
static socket_t *_lookup_sock_t(xbee_sock_t sock)
{
    ...
    //printf("%s: failed to find xbee_sock_t 0x%04X\n",
               //  __FUNCTION__, sock);
    return NULL;
}
...
static socket_t *_lookup_socket_id(xbee_dev_t *xbee, uint8_t socket_id)
{
    ...
    //printf("%s: failed to find socket_id 0x%02X\n",
                             ...
                             xbee_sock_notify_fn notify_handler)
{
    ...
    //printf("%s: created socket 0x%04X\n", __FUNCTION__, _xbee_sock_t(s));
                      ...
                      xbee_sock_receive_fn receive_handler)
{
    ...
    //printf("%s: connect socket 0x%02X frame 0x%02X result %d\n", __FUNCTION__,
                               ...
                               socket_t **sock)
{
    ...
    //printf("%s: socket 0x%02X frame 0x%02X result %d\n", __FUNCTION__,
                   ...
                   const void *payload, size_t payload_len)
{
    ...
    //printf("%s: %u bytes in frame 0x%02X  sock 0x%02X  result %d\n",
...
int xbee_sock_recv(xbee_sock_t socket,const void *payload,
					size_t reqested_len,  uint32_t Flag)
{
	int lRecvBytes = 0;
	(void)Flag;

	if(recv.hw_buff_cur_off != HW_BUFF_BASE_ADDR) // there is some data available get it
	{
		if(recv.sw_buff_cur_off == BUFFER_MAX_SIZE )
			recv.sw_buff_cur_off  = SW_BUFF_BASE_ADDR; // we took everything from buffer

		if(reqested_len > lteMAX_RECV_BYTES)
			reqested_len = lteMAX_RECV_BYTES;

		if (reqested_len > (recv.hw_buff_cur_off - recv.sw_buff_cur_off ) ) //current ask length will overflow then availabe size so give whatever we have left
					reqested_len  = recv.hw_buff_cur_off - recv.sw_buff_cur_off;

		memcpy(payload,recv.pkt + recv.sw_buff_cur_off,reqested_len);
		lRecvBytes = reqested_len;
		recv.sw_buff_cur_off = recv.sw_buff_cur_off + reqested_len;

		/*After shifting sw pointer is hw and sw are both at same location ?\
		 * If yes then there is no pending data from hw side
		 * so lets start to fill and get from BASE_ADDR
		 * */
		if(recv.sw_buff_cur_off == recv.hw_buff_cur_off)
		{
			recv.sw_buff_cur_off = SW_BUFF_BASE_ADDR;
			recv.hw_buff_cur_off = HW_BUFF_BASE_ADDR;
		}
	}
	else // data is not available yes to receive
	{
		//printf("DBG:Rx data not available \n");
...
{
    /*//printf("%s: type 0x%02X  frame 0x%02X  sock 0x%02X  status 0x%02X\n",
                              ...
                              const xbee_frame_sock_shared_resp_t FAR *frame)
{
    ...
    if (s == NULL) {
        /*//printf("%s: couldn't match socket_id 0x%02X\n",
                     __FUNCTION__, frame->socket_id);*/
        return -ENOENT;
    }

    if (s->type != XBEE_SOCK_SOCKET_TYPE_ATTACH) {
        //printf("%s: socket type 0x%02X not ATTACH\n",
                   //  __FUNCTION__, s->type);
        return -EPERM;
    }
                              ...
                              uint16_t length)
{
    ...
    //printf("%s: frame 0x%02X  sock 0x%02X  opt 0x%02X  status 0x%02X\n",
...
{
    //printf("%s: listen 0x%02X  new client 0x%02X from 0x%08X:%u\n",
    ...
    if (listen->type != XBEE_SOCK_SOCKET_TYPE_LISTEN) {
        //printf("%s: socket type 0x%02X not LISTEN\n",
                  //   __FUNCTION__, listen->type);
        return -EPERM;
    }

    //printf("%s: listen xbee_sock_t 0x%04X\n",
    ...
    if (client == NULL) {
        //printf("%s: cannot allocate sock for client 0x%02X\n",
        ...
        //printf("%s: allocated sock 0x%04X for client 0x%02X\n",
                    // _//_FUNCTION__, _xbee_sock_t(client), frame->sock_client_id);
    }
...
static int _common_receive(xbee_dev_t *xbee,
                           ...
                           socket_t **socket)
{
    ////printf("%s: socket 0x%02X:  %d bytes\n",
   ...
   // //printf("%s: xbee_sock_t 0x%04X\n", __FUNCTION__, _xbee_sock_t(s));

    if (s->type != type_check) {
        //printf("%s: socket type 0x%02X != 0x%02X\n",
...
{
    //printf("%s: socket 0x%02X  state 0x%02x\n", __FUNCTION__,
                            ...
                            uint16_t length, void FAR *context)
{
    ...
    case XBEE_FRAME_SOCK_RECEIVE:
    	////printf("XBEE_FRAME_SOCK_RECEIVE 0x%x,\n",*(const uint8_t *)rawframe);
    	recv_cb_arrived = 1;
    ...
    case XBEE_FRAME_SOCK_STATE:
    	//printf("XBEE_FRAME_SOCK_STATE 0x%x,\n",*(const uint8_t *)rawframe);
...
{
  uint32_t ip = 0;
  printf("Resolving FQDN .. \n");
  while (!ip) {
    sendAtRequest(xbee, "LA", fqdnStr, resolveFQDNCallback, &ip);
    printf(".");
  }
  printf("\n");
  return ip;
 ...
 // portTickType openTimeout; //< Var to hold the timeout
  ...
  if (request > 0) {
   ...
   // vTaskSetTimeOutState(&x_timeout);
    ...
    //openTimeout = pdMS_TO_TICKS(50); //< In RTOS tick
                            ...
                            uint8_t frame_type, uint8_t message)
{
    ...
    case XBEE_FRAME_TX_STATUS:          // message is an XBEE_TX_DELIVERY_xxx
        if (message == 0) { //success
        	tx_done = 1;
        	//printf("XBEE_FRAME_TX_STATUS tx Sucess ... \n");
        } else { //failed
        	printf("\nSocket 0x%04X: XBEE_TX_DELIVERY=0x0%x\n",socket, message);
        	tx_done = -1;
        }
        break;
   ...
   // message is an XBEE_SOCK_STATUS_xxx
        if (message != 0) {
...
void receive_callback(xbee_sock_t socket, uint8_t status,
                             const void *payload, size_t payload_length)
{
    ...
    if(recv_cb_arrived)
    {
    	...
    	// full or new data will be like current off has only 10 bytes left but received 512 byte so can not fill further
    	if(recv.hw_buff_cur_off == BUFFER_MAX_SIZE && (payload_length + recv.hw_buff_cur_off ) > BUFFER_MAX_SIZE) //buffer full
    	{
    		recv.hw_buff_cur_off = HW_BUFF_BASE_ADDR; // Start filling from beaning as room is full
    	}
    	else //still 8k buffer has room
    	{
    		// do nothing and copy below
    	}

    	memcpy(recv.pkt + recv.hw_buff_cur_off,payload,payload_length);
    	recv.hw_buff_cur_off  = recv.hw_buff_cur_off + payload_length;

    	recv_cb_arrived = 0; //i filled hw buffer let new data come in now
    	counter ++;
    	if(!buff_available)
    		buff_available  =1 ;
    }
...
void option_callback(xbee_sock_t socket, uint8_t option_id,
...
/***************************************************************************//**
 * @brief If XBeee sends an IM response this updates the IMEI number
 * @param response The AT command response from IM issued by getIMEI()
...
int xbeeIMEICallback(const xbee_cmd_response_t FAR *response)
{
	...
	if (((response->flags & XBEE_CMD_RESP_MASK_STATUS) == XBEE_AT_RESP_SUCCESS)
		&& response->value_length == 15) {
		if (xbee == NULL) {
			printf("Xbee err\n");
			return -EINVAL;
		}

		memcpy(response->context, response->value_bytes, 15);

	}
	 if ((response->flags & XBEE_CMD_RESP_MASK_STATUS) == XBEE_AT_RESP_ERROR) {
	    /* Not a valid IMEI */
	    printf("\fError w\\ in getting IMEI");
...
/***************************************************************************//**
 * @brief Sends out an IM command to update connection status
 ******************************************************************************/
uint8_t* getIMEI(xbee_dev_t *xbee) {

	static uint8_t imei[15] = { 0 };

	while (imei[0] == 0) {
		sendAtRequest(xbee, "IM", NULL, xbeeIMEICallback, imei);
		vTaskDelay(10);
	}
	printf("\n");
	return imei;

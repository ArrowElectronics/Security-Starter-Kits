...
then
	...
	echo "./Gateway_Demo_aiml.sh Demo.config"
	exit
fi
...
run_greengrass_demo()
{
	...
	python Version_Aiml.py --endpoint "$endpoint" --rootCA "$rootca" --cert "$switch_cert" --key "$switch_key" --mode both &
	version_pid="$!"
...
{
	echo "Hello, $(whoami)!"	
	echo "########  Welcome to IoT iMX8X SSK Security Demos [] ##########"
	...
	case "$REPLY" in
		...
		n|N)
			echo "******************Please Run SSK_Suit_Configuration Script before this procees**********************"
			;;

...
open class BleCommandTransfer {
    ...
    companion object {
        ...
        fun generateSecureRandom(): ByteArray? {
            ...
            //Log.e("BleCommandTransfer", "Hex data " + byteArrayToHex(randomNumber))
            return randomNumber
        }
        ...
        fun signatureVerificationFail(): ByteArray? {
            return byteArrayOf(0x04)
        }
        ...
        fun gatewaySecretFail(): ByteArray? {
        ...
        fun generateDataFrame(data: ByteArray): ByteArray {
            ...
            //Log.e(TAG, "All data frame $actualDataInHex")
            val totalDataLength = data.size + 4
            val MSBLSBLength = integerToHex(totalDataLength, 4)

            val crc16 = CalculateCRC16.crc16_U(data).toInt()
            val crc16ToInt = integerToHex(crc16,4)
            //Log.e(TAG, "All data frame $crc16ToInt")
            val dataFrame = MSBLSBLength + actualDataInHex + crc16ToInt
            //Log.e(TAG, "All data frame $dataFrame")
            return hexToByteArray(dataFrame)
        }


        fun getFrameLengthForLog(data: ByteArray): Int {
            var lengthData = byteArrayToHex(byteArrayOf(data[0], data[1]))
            return hexToInteger(lengthData)
        }

        fun getFrameLengthForLogFromHex(data: String): Int {
            val lengthData = data.take(4)
            return hexToInteger(lengthData)
        }


        fun generateDataFrameInHEX(data: ByteArray): String {
            val actualDataInHex = byteArrayToHex(data)
            //Log.e(TAG, "All data frame $actualDataInHex")
            val totalDataLength = data.size + 4
            val MSBLSBLength = integerToHex(totalDataLength, 4)
        ...
        @JvmStatic
        fun getPublicKeyHeaderInHEX(): String {
            return "3059301306072a8648ce3d020106082a8648ce3d030107034200"
        }
